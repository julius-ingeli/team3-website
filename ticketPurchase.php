...
if(!is_numeric($a))    
{
    echo "<h2 class='h1 col-6 offset-3 text-center'> You need to <a href='login.php'>log in</a> or <a href='signup.php'>sign up</a>. <br> </h2>";
}
else
{
    switch ($tier)
    ...
    if(isset($_POST['submit']))
    {
        ...
        $ticketSQL = "INSERT INTO ticketInfo (assocAcc, dop, tier, active) VALUES ('$a', '$dop', '$tier', '0')";
        $paymentSQL = "INSERT INTO paymentInfo (id, cardNum, doe, cvv, street, city, postal, province, country, cardholder) VALUES ('$a', '$cardNum', '$doe', '$cvv', '$street', '$city', '$postal', '$province', '$country', '$cardholder')";

        if($conn->query($ticketSQL) === TRUE && $conn->query($paymentSQL) === TRUE)
        {
            echo "<h2 class='h1 col-4 offset-4 text-center'> <hr> Order successful! <br> You can find more details in an <br> E-mail that we sent you! <hr></h2>";
        }
        else 
        {
